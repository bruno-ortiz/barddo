...
class Migration(DataMigration):
    def forwards(self, orm):
        for user in orm.BarddoUser.objects.all():
            owned_publishers = orm['publishing.PublishingHouse'].objects.filter(owner=user).count()
            num_publishers = orm['publishing.PublishingHouse'].objects.filter(publishers=user).count()
            user.is_publisher = (owned_publishers + num_publishers) > 0
            user.save()
    ...
    models = {
        ...
        u'accounts.barddouserprofile': {
            ...
            'birth_date': ('django.db.models.fields.DateField', [], {'default': 'datetime.datetime(2014, 9, 3, 0, 0)'}),
        ...
        u'publishing.publishinghouse': {
            ...
            'publishers': ('django.db.models.fields.related.ManyToManyField', [],
                           {'blank': 'True', 'related_name': "'publishing_group'", 'null': 'True', 'symmetrical': 'False', 'to': u"orm['accounts.BarddoUser']"})
        }
    }

    complete_apps = ['publishing', 'accounts']
    symmetrical = True
